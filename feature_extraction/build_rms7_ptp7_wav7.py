#!/usr/bin/env python3
"""
Build pass-level RMS7 + PTP7 + wavelet-energy-ratio-7 features.
...
def build_one_run(project_root: Path, run: str, out_dir: Path) -> None:
    ...
    X_sel = X[:, KEEP_IDX].astype(np.float32)
    ...
    feat_names = np.asarray(
        KEEP_NAMES + [f"WAVELET_ENERGY_RATIO_CH{i}" for i in range(1, 8)],
        dtype=object,
    )

    out_dir.mkdir(parents=True, exist_ok=True)
    np.savez_compressed(
        out_dir / f"{run}_passlevel_rms7_ptp7_wav7.npz",
        X=X_out,
        y=y,
        pass_idx=pass_idx,
...
def main() -> None:
    ...
    p.add_argument(
        "--out_dir",
        type=Path,
        default=Path("dataset/passlevel_tree_select/selected_rms7_ptp7_wav7"),
    )
    args = p.parse_args()
    ...
    for run in args.runs:
        ...
        print(f"[OK] built {run} -> {out_dir / f'{run}_passlevel_rms7_ptp7_wav7.npz'}")
