...
RMS7 + Feat_4 + HF_ENERGY_RATIO_CH2.
"""
...
def build_one_run(project_root: Path, run: str, out_dir: Path, channel_idx: int) -> None:
    ...
    np.savez_compressed(
        out_dir / f"{run}_passlevel_rms7_plus_feat4_plus_hf2.npz",
        X=X_out,
        y=y,
        pass_idx=pass_idx,
...
def main() -> None:
    ...
    p.add_argument(
        "--out_dir",
        type=Path,
        default=Path("dataset/passlevel_tree_select/selected_rms7_plus_feat4_plus_hf2"),
    )
    p.add_argument("--channel_idx", type=int, default=1)
    args = p.parse_args()
    ...
    for run in args.runs:
        ...
        print(f"[OK] built {run} -> {out_dir / f'{run}_passlevel_rms7_plus_feat4_plus_hf2.npz'}")
