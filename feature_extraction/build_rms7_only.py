#!/usr/bin/env python3
"""
Build pass-level pure RMS7 features from base_td28.

This keeps only the 7 RMS dimensions:
Feat_3, Feat_7, Feat_11, Feat_15, Feat_19, Feat_23, Feat_27
"""
...
def build_one_run(project_root: Path, run: str, out_dir: Path) -> None:
    ...
    X_out = X[:, RMS_KEEP_IDX].astype(np.float32)
    feat_names = np.asarray(RMS_KEEP_NAMES, dtype=object)

    out_dir.mkdir(parents=True, exist_ok=True)
    np.savez_compressed(
        out_dir / f"{run}_passlevel_rms7.npz",
        X=X_out,
        y=y,
        pass_idx=pass_idx,
...
def main() -> None:
    ...
    p.add_argument(
        "--out_dir",
        type=Path,
        default=Path("dataset/passlevel_tree_select/selected_rms7"),
    )
    args = p.parse_args()
    ...
    for run in args.runs:
        ...
        print(f"[OK] built {run} -> {out_dir / f'{run}_passlevel_rms7.npz'}")
