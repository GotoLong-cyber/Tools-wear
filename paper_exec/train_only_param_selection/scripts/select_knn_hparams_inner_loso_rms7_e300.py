#!/usr/bin/env python3
"""Inner LOSO validation for pure RMS7 @ epoch300 retrieval hyperparameter selection."""
...
def main() -> None:
    ...
    parser.add_argument("--k_grid", type=str, default="3,5,7,10,15")
    parser.add_argument("--beta_grid", type=str, default="0.0,0.3,0.5,0.7,1.0")
    parser.add_argument("--late_q_grid", type=str, default="0.8")
    ...
    for spec in FOLD_SPECS:
        ...
        for library_run, query_run in [(src_runs[0], src_runs[1]), (src_runs[1], src_runs[0])]:
            ctx = prepare_inner_context(project_root, spec, library_run, query_run, results_subdir="paper_exec_inner_loso_param_select_rms7_e300")
    ...
    selected_json = {
        ...
        "note": "Selected on pure RMS7 @ epoch300 without using outer test folds.",
    }
    ...
    summary_md = project_root / "paper_exec" / "train_only_param_selection" / "manifests" / "InnerLOSO选参汇总_RMS7_E300.md"
    lines = [
