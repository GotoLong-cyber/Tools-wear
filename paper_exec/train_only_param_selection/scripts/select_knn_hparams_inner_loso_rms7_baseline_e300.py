#!/usr/bin/env python3
"""Inner LOSO validation for pure RMS7 baseline+KNN @ epoch300.

This script is the paper-grade legal selection path for the final mainline:
pure RMS7 + Baseline + KNN (no TMA).
"""
...
FOLD_SPECS = [
    FoldSpec(
        ...
        train_runs="c1,c4",
        test_runs="c6",
        runtime_root="dataset/passlevel_tree_select/runtime_rms7_baseline_clean_fold1_gpu3",
        checkpoint_path="checkpoints/forecast_PHM_c1c4_to_c6_rms7_BaselineClean_meanagg_dual_seed2026_e300_bt96_gpu3_novalfix_r7e_fold1_e300_timer_xl_PHM_MergedMultivariateNpy_sl96_it96_ot16_lr0.0001_bt96_wd0_el8_dm1024_dff2048_nh8_cosTrue_test_0/checkpoint.pth",
    ),
    FoldSpec(
        fold="fold2",
        train_runs="c4,c6",
        test_runs="c1",
        runtime_root="dataset/passlevel_tree_select/runtime_rms7_baseline_clean_fold2_gpu0",
        checkpoint_path="checkpoints/forecast_PHM_c4c6_to_c1_rms7_BaselineClean_meanagg_dual_seed2026_e300_bt96_gpu0_novalfix_r7e_fold2_e300_timer_xl_PHM_MergedMultivariateNpy_sl96_it96_ot16_lr0.0001_bt96_wd0_el8_dm1024_dff2048_nh8_cosTrue_test_0/checkpoint.pth",
    ),
    FoldSpec(
        fold="fold3",
        train_runs="c1,c6",
        test_runs="c4",
        runtime_root="dataset/passlevel_tree_select/runtime_rms7_baseline_clean_fold3_gpu1",
        checkpoint_path="checkpoints/forecast_PHM_c1c6_to_c4_rms7_BaselineClean_meanagg_dual_seed2026_e300_bt96_gpu1_novalfix_r7e_fold3_e300_timer_xl_PHM_MergedMultivariateNpy_sl96_it96_ot16_lr0.0001_bt96_wd0_el8_dm1024_dff2048_nh8_cosTrue_test_0/checkpoint.pth",
    ),
]
    ...
    for late_q in q_grid:
        ...
        for k in k_grid:
            ...
            if cache_key not in knn_cache:
                ...
                knn_cache[cache_key] = delta_knn
            delta_knn = knn_cache[cache_key]
...
def main() -> None:
    ...
    parser.add_argument("--k_grid", type=str, default="3,5,7,10")
    parser.add_argument("--beta_grid", type=str, default="0.3,0.5,0.7,1.0")
    parser.add_argument("--late_q_grid", type=str, default="0.0,0.5,0.8")
    ...
    for spec in FOLD_SPECS:
        ...
        for library_run, query_run in [(src_runs[0], src_runs[1]), (src_runs[1], src_runs[0])]:
            ctx = prepare_inner_context(
                ...
                library_run,
                query_run,
                results_subdir="paper_exec_inner_loso_param_select_rms7_baseline_e300",
            )
    ...
    selected_json = {
        ...
        "selection_source": str(summary_csv),
        "note": "Selected on pure RMS7 baseline+KNN @ epoch300 without using outer test folds.",
    }
    ...
    summary_md = project_root / "paper_exec" / "train_only_param_selection" / "manifests" / "InnerLOSO选参汇总_RMS7_Baseline_E300.md"
    lines = [
