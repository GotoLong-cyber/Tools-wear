...
RESULTS_SUBDIR="${RESULTS_SUBDIR:-20260409_r7e_${FOLD}_e${TRAIN_EPOCHS}}"
MODEL_TAG="${MODEL_TAG:-r7e_${FOLD}_e${TRAIN_EPOCHS}}"
...
CKPT="$(find "${PROJECT_ROOT}/checkpoints" -path "*PHM_${TRAIN_RUNS//,/}_to_${TEST_RUNS}_rms7_BaselineClean_${WEAR_AGG}agg_dual_seed2026_e${TRAIN_EPOCHS}_bt96_gpu${GPU_ID}_novalfix_${MODEL_TAG}*checkpoint.pth" | head -n 1)"
test -n "${CKPT}"

"${PYTHON_BIN}" "${PROJECT_ROOT}/feature_alignment_diagnosis/scripts/evaluate_headonly_fold.py" \
  --project_root "${PROJECT_ROOT}" \
  --runtime_root "${PROJECT_ROOT}/dataset/passlevel_tree_select/runtime_rms7_baseline_clean_${FOLD}_gpu${GPU_ID}" \
...
echo "[RMS7-BASELINE][OK] ${FOLD} e${TRAIN_EPOCHS} complete"
