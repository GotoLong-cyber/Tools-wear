...
FOLD="${FOLD}" \
GPU_ID="${GPU_ID}" \
RESULTS_SUBDIR="20260408_r7p7b_${FOLD}" \
...
MODEL_ID_EXTRA_SUFFIX="r7p7b_${FOLD}" \
bash "${PROJECT_ROOT}/paper_exec/scripts/run_clean_timer_fold.sh"

CKPT="$(find "${PROJECT_ROOT}/checkpoints" -path "*PHM_${TRAIN_RUNS//,/}_to_${TEST_RUNS}_rms7ptp_BaselineClean_${WEAR_AGG}agg_dual_seed2026_e${TRAIN_EPOCHS}_bt96_gpu${GPU_ID}_novalfix_r7p7b_${FOLD}*checkpoint.pth" | head -n 1)"
test -n "${CKPT}"
...
"${PYTHON_BIN}" "${PROJECT_ROOT}/feature_alignment_diagnosis/scripts/evaluate_headonly_fold.py" \
  ...
  --results_subdir "20260408_r7p7bh_${FOLD}" \
...
echo "[RMS7PTP7-BASELINE][OK] ${FOLD} complete"
