...
run_variant rms7ptpwav_tma "20260408_r7p7wt_${FOLD}" "r7p7wt_${FOLD}"
CKPT="$(find_ckpt "PHM_${TRAIN_RUNS//,/}_to_${TEST_RUNS}_rms7ptpwav_TMAClean_${WEAR_AGG}agg_dual_seed2026_e${TRAIN_EPOCHS}_bt96_gpu${GPU_ID}_novalfix_r7p7wt_${FOLD}")"
test -n "${CKPT}"
eval_headonly \
  "${PROJECT_ROOT}/dataset/passlevel_tree_select/runtime_rms7_ptp7_wav7_tma_clean_${FOLD}_gpu${GPU_ID}" \
  "${CKPT}" \
  "20260408_r7p7wth_${FOLD}" \
  "r7p7wt_${FOLD}" \
  21

run_variant rms7ptpwav_retrieval "20260408_r7p7wr_${FOLD}" "r7p7wr_${FOLD}"
CKPT="$(find_ckpt "PHM_${TRAIN_RUNS//,/}_to_${TEST_RUNS}_rms7ptpwav_TMAClean_${WEAR_AGG}agg_dual_seed2026_e${TRAIN_EPOCHS}_bt96_gpu${GPU_ID}_novalfix_r7p7wr_${FOLD}")"
test -n "${CKPT}"
eval_knn \
  "${PROJECT_ROOT}/dataset/passlevel_tree_select/runtime_rms7_ptp7_wav7_retrieval_clean_${FOLD}_gpu${GPU_ID}" \
  "${CKPT}" \
  "20260408_r7p7wk_${FOLD}" \
  "r7p7wr_${FOLD}" \
  21

echo "[R7P7W-POST][OK] ${FOLD} complete"
