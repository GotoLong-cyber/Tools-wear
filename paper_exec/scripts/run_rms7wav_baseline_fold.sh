...
FOLD="${FOLD}" \
GPU_ID="${GPU_ID}" \
RESULTS_SUBDIR="20260408_r7w_${FOLD}" \
...
MODEL_ID_EXTRA_SUFFIX="r7w_${FOLD}" \
bash "${PROJECT_ROOT}/paper_exec/scripts/run_clean_timer_fold.sh"

CKPT="$(find "${PROJECT_ROOT}/checkpoints" -path "*PHM_${TRAIN_RUNS//,/}_to_${TEST_RUNS}_rms7wav_BaselineClean_${WEAR_AGG}agg_dual_seed2026_e${TRAIN_EPOCHS}_bt96_gpu${GPU_ID}_novalfix_r7w_${FOLD}*checkpoint.pth" | head -n 1)"
test -n "${CKPT}"

"${PYTHON_BIN}" "${PROJECT_ROOT}/feature_alignment_diagnosis/scripts/evaluate_headonly_fold.py" \
  --project_root "${PROJECT_ROOT}" \
  --runtime_root "${PROJECT_ROOT}/dataset/passlevel_tree_select/runtime_rms7_wav7_baseline_clean_${FOLD}_gpu${GPU_ID}" \
  --checkpoint_path "${CKPT}" \
  --results_subdir "20260408_r7wh_${FOLD}" \
...
echo "[RMS7WAV7-BASELINE][OK] ${FOLD} complete"
