...
RESULTS_SUBDIR="20260410_r7p7e300_${FOLD}"
RESULTS_HEAD_SUBDIR="20260410_r7p7e300h_${FOLD}"
RESULTS_KNN_SUBDIR="20260410_r7p7bk300_${FOLD}"
MODEL_TAG="r7p7e300_${FOLD}"
KNN_TAG="r7p7bk300_${FOLD}"
...
CKPT="$(find "${PROJECT_ROOT}/checkpoints" -path "*PHM_${TRAIN_RUNS//,/}_to_${TEST_RUNS}_rms7ptp_BaselineClean_${WEAR_AGG}agg_dual_seed2026_e${TRAIN_EPOCHS}_bt96_gpu${GPU_ID}_novalfix_${MODEL_TAG}*checkpoint.pth" | head -n 1)"
test -n "${CKPT}"

"${PYTHON_BIN}" "${PROJECT_ROOT}/feature_alignment_diagnosis/scripts/evaluate_headonly_fold.py" \
  --project_root "${PROJECT_ROOT}" \
  --runtime_root "${PROJECT_ROOT}/dataset/passlevel_tree_select/runtime_rms7_ptp7_baseline_clean_${FOLD}_gpu${GPU_ID}" \
...
"${PYTHON_BIN}" "${PROJECT_ROOT}/feature_alignment_diagnosis/scripts/evaluate_fold1_knn_delta_retrieval.py" \
  --project_root "${PROJECT_ROOT}" \
  --runtime_root "dataset/passlevel_tree_select/runtime_rms7_ptp7_baseline_clean_${FOLD}_gpu${GPU_ID}" \
...
echo "[RMS7PTP7-E300-CHAIN][OK] ${FOLD}"
