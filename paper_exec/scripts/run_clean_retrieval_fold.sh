...
# Clean formal retrieval inference for a single fold.
# Uses clean retrieval-backbone checkpoints and fixed retrieval hyperparameters.
...
LOG_FILE="${ROUND_DIR}/master_clean_retrieval_${FOLD}_gpu${GPU_ID}.log"
CMD_FILE="${ROUND_DIR}/cmd_clean_retrieval_${FOLD}_gpu${GPU_ID}.txt"
: > "${CMD_FILE}"
for arg in "${CMD[@]}"; do
...
echo "[DONE] clean retrieval ${FOLD}"
