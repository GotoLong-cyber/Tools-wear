...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # zml
        self.target_only = getattr(configs, "target_only", False)
        self.target_idx = getattr(configs, "target_idx", -1)
    ...
    def forecast(self, x, x_mark, y_mark):
        ...
        # ---- ONLY predict target channel (wear) ----
        if self.target_only:
            ...
            # [B, N, P] -> [B, L, 1] where L = N*P
            dec_out = dec_out.reshape(B, N * dec_out.shape[-1], 1)

            if self.use_norm:
                # 只取目标通道的 mean/std，避免 8维广播
                means_t = means[:, :, target_idx:target_idx + 1]  # [B,1,1]
        ...
        # ---- original multivariate path (keep as-is) ----
        dec_out = self.head(embed_out)  # [B, C*N, P]
        dec_out = dec_out.reshape(B, C, N, -1).reshape(B, C, -1)  # [B, C, N*P]
