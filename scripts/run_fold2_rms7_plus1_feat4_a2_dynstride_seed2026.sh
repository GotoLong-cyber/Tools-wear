...
# A2 revised: causal train-only dynamic-stride augmentation in time domain.
...
"${PYTHON_BIN}" - <<'PY'
import torch
assert torch.cuda.is_available(), "CUDA unavailable: revised A2 must run on GPU."
print("[GPU][OK] cuda available")
PY

GPU_ID="${GPU_ID:-1}"
SELECTED_DIR="${PROJECT_ROOT}/dataset/passlevel_tree_select/selected_rms7_plus_feat4"
RUNTIME_DIR="${PROJECT_ROOT}/dataset/passlevel_tree_select/runtime_rms7_plus_feat4_a2_dynstride_fold2_gpu${GPU_ID}"
...
wear_agg="max"
num_workers=0
train_stride_candidates="${TRAIN_STRIDE_CANDIDATES:-1,2,3}"
train_stride_quantiles="${TRAIN_STRIDE_QUANTILES:-0.5,0.25}"
...
model_id="PHM_c4c6_to_c1_rms7_plus_feat4_A2dynstride_dual_seed${seed}_e${train_epochs}_bt${batch_size}_gpu${GPU_ID}"
run_log="${PROJECT_ROOT}/results/longrun_${model_id}.log"

echo "[RUN] ${model_id}"
echo "[RUN] log=${run_log}"
echo "[RUN] train_stride_candidates=${train_stride_candidates} train_stride_quantiles=${train_stride_quantiles}"
