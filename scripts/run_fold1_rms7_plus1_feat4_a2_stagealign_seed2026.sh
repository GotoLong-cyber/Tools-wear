...
# A2 Fold-1 对照实验（磨损阶段对齐切片）
# train/val: c1,c4 ; test: c6
...
SELECTED_DIR="${PROJECT_ROOT}/dataset/passlevel_tree_select/selected_rms7_plus_feat4_a2_stagealign_fold1"
RUNTIME_DIR="${PROJECT_ROOT}/dataset/passlevel_tree_select/runtime_rms7_plus_feat4_a2_stagealign_fold1_gpu1"
mkdir -p "${SELECTED_DIR}" "${RUNTIME_DIR}" "${PROJECT_ROOT}/results"

# Build stage-aligned data for c1/c4; keep c6 untouched.
"${PYTHON_BIN}" "${PROJECT_ROOT}/feature_alignment_diagnosis/scripts/build_a2_stage_aligned_fold1_data.py" \
  ...
  --output_dir "dataset/passlevel_tree_select/selected_rms7_plus_feat4_a2_stagealign_fold1" \
  --runs c1 c4 c6 \
  --train_runs c1 c4 \
...
model_id="PHM_c1c4_to_c6_rms7_plus_feat4_A2stagealign_dual_seed${seed}_e${train_epochs}_bt${batch_size}_gpu1"
