...
# A2 ablation C (StageWeightOnly-S2): keep stride=1, only duplicate high-wear train windows.
# fold2: train/val=c4,c6 ; test=c1
...
"${PYTHON_BIN}" - <<'PY'
import torch
assert torch.cuda.is_available(), "CUDA unavailable: A2-StageWeightOnly-S2 must run on GPU."
print("[GPU][OK] cuda available")
PY

GPU_ID="${GPU_ID:-1}"
RESULTS_SUBDIR="${RESULTS_SUBDIR:-20260323_A2_stageweightonly_s2}"
...
RUNTIME_DIR="${PROJECT_ROOT}/dataset/passlevel_tree_select/runtime_rms7_plus_feat4_a2_stageweight_s2_fold2_gpu${GPU_ID}"
...
wear_agg="max"
num_workers=0
train_stride_candidates="${TRAIN_STRIDE_CANDIDATES:-1,2}"
train_stride_quantiles="${TRAIN_STRIDE_QUANTILES:-0.5}"
train_window_weight_quantile="${TRAIN_WINDOW_WEIGHT_QUANTILE:-0.5}"
train_window_weight_seed="${TRAIN_WINDOW_WEIGHT_SEED:-2026}"
...
model_id="PHM_c4c6_to_c1_rms7_plus_feat4_A2stageweightS2_dual_seed${seed}_e${train_epochs}_bt${batch_size}_gpu${GPU_ID}"
run_log="${ROUND_DIR}/longrun_${model_id}.log"

echo "[RUN] ${model_id}"
echo "[RUN] log=${run_log}"
echo "[RUN] results_subdir=${RESULTS_SUBDIR}"
echo "[RUN] train_window_weight_quantile=${train_window_weight_quantile} train_window_weight_seed=${train_window_weight_seed}"
...
"${PYTHON_BIN}" -u run.py \
  ...
  --train_stride_candidates "${train_stride_candidates}" \
  --train_stride_quantiles "${train_stride_quantiles}" \
  --train_stride_use_monotonic_wear 1 \
  --train_window_weight_policy stage_weight_only \
  --train_window_weight_quantile "${train_window_weight_quantile}" \
  --train_window_weight_seed "${train_window_weight_seed}" \
