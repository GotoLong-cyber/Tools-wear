...
# A2 ablation A (Random-S2): random stride allocation matching slope-guided counts.
...
"${PYTHON_BIN}" - <<'PY'
import torch
assert torch.cuda.is_available(), "CUDA unavailable: A2-Random-S2 must run on GPU."
print("[GPU][OK] cuda available")
PY

GPU_ID="${GPU_ID:-0}"
SELECTED_DIR="${PROJECT_ROOT}/dataset/passlevel_tree_select/selected_rms7_plus_feat4"
RUNTIME_DIR="${PROJECT_ROOT}/dataset/passlevel_tree_select/runtime_rms7_plus_feat4_a2_randoms2_fold1_gpu${GPU_ID}"
mkdir -p "${RUNTIME_DIR}" "${PROJECT_ROOT}/results"
...
train_stride_random_seed="${TRAIN_STRIDE_RANDOM_SEED:-2026}"
...
model_id="PHM_c1c4_to_c6_rms7_plus_feat4_A2randomS2_dual_seed${seed}_e${train_epochs}_bt${batch_size}_gpu${GPU_ID}"
run_log="${PROJECT_ROOT}/results/longrun_${model_id}.log"

echo "[RUN] ${model_id}"
echo "[RUN] log=${run_log}"
echo "[RUN] train_stride_candidates=${train_stride_candidates} train_stride_quantiles=${train_stride_quantiles} random_seed=${train_stride_random_seed}"
...
"${PYTHON_BIN}" -u run.py \
  ...
  --train_stride_random_seed "${train_stride_random_seed}" \
