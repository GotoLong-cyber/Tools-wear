...
# Official confirmation: A2-final = Random-S2, fold1 c1,c4 -> c6
...
"${PYTHON_BIN}" - <<'PY'
import torch
assert torch.cuda.is_available(), "CUDA unavailable: A2-final must run on GPU."
print("[GPU][OK] cuda available")
PY

GPU_ID="${GPU_ID:-0}"
RESULTS_SUBDIR="${RESULTS_SUBDIR:-20260323_A2Final_feat4}"
...
RUNTIME_DIR="${PROJECT_ROOT}/dataset/passlevel_tree_select/runtime_rms7_plus_feat4_a2_final_fold1_gpu${GPU_ID}"
...
model_id="PHM_c1c4_to_c6_rms7_plus_feat4_A2Final_dual_seed${seed}_e${train_epochs}_bt${batch_size}_gpu${GPU_ID}"
