...
# Fold-1 正式实验（严格同口径）
# 目标：RMS7 + 每通道再加 1 个特征（range）= 14 维特征
# 说明：
...
SELECTED_DIR="${PROJECT_ROOT}/dataset/passlevel_tree_select/selected_rms7_plus_range7"
RUNTIME_DIR="${PROJECT_ROOT}/dataset/passlevel_tree_select/runtime_rms7_plus_range7_gpu1"
mkdir -p "${SELECTED_DIR}" "${RUNTIME_DIR}" "${PROJECT_ROOT}/results"

# 从 td28 中抽取：每通道 RMS + range
# td28 顺序为 [mean, std, rms, range] * 7
# 0-based 索引：
# RMS   -> [2, 6, 10, 14, 18, 22, 26]
# range -> [3, 7, 11, 15, 19, 23, 27]
"${PYTHON_BIN}" - <<'PY'
...
selected_dir = project / "dataset/passlevel_tree_select/selected_rms7_plus_range7"
selected_dir.mkdir(parents=True, exist_ok=True)

idx = np.array([2, 6, 10, 14, 18, 22, 26, 3, 7, 11, 15, 19, 23, 27], dtype=np.int64)
...
for run in ["c1", "c4", "c6"]:
    ...
    np.savez(
        selected_dir / f"{run}_passlevel_rms7_plus_range7.npz",
        X=X,
        y=y,
        pass_idx=p,
...
(selected_dir / "keep_features_rms7_plus_range7.txt").write_text("\n".join(names) + "\n", encoding="utf-8")
print("[OK] selected feature count =", len(names))
print("[OK] features =", names)
PY

# 适配 PHM_MergedMultivariateNpy 的命名约定
ln -sfn "${SELECTED_DIR}/c1_passlevel_rms7_plus_range7.npz" "${RUNTIME_DIR}/c1_passlevel_full133.npz"
ln -sfn "${SELECTED_DIR}/c4_passlevel_rms7_plus_range7.npz" "${RUNTIME_DIR}/c4_passlevel_full133.npz"
ln -sfn "${SELECTED_DIR}/c6_passlevel_rms7_plus_range7.npz" "${RUNTIME_DIR}/c6_passlevel_full133.npz"
...
batch_size=96
learning_rate=1e-4
# 默认拉长训练轮次；可通过环境变量覆盖，如 TRAIN_EPOCHS=600
...
n_vars=15  # 14维特征 + 1维wear

model_id="PHM_c1c4_to_c6_rms7_plus_range7_dual_seed${seed}_e${train_epochs}_bt${batch_size}_gpu1"
