...
# A2 ablation B (S2): causal train-only dynamic-stride augmentation in time domain.
...
"${PYTHON_BIN}" - <<'PY'
import torch
assert torch.cuda.is_available(), "CUDA unavailable: A2-S2 must run on GPU."
print("[GPU][OK] cuda available")
PY

GPU_ID="${GPU_ID:-1}"
SELECTED_DIR="${PROJECT_ROOT}/dataset/passlevel_tree_select/selected_rms7_plus_feat4"
RUNTIME_DIR="${PROJECT_ROOT}/dataset/passlevel_tree_select/runtime_rms7_plus_feat4_a2_dynstride_s2_fold2_gpu${GPU_ID}"
...
model_id="PHM_c4c6_to_c1_rms7_plus_feat4_A2dynstrideS2_dual_seed${seed}_e${train_epochs}_bt${batch_size}_gpu${GPU_ID}"
