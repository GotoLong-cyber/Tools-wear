...
"${PYTHON_BIN}" - <<'PY'
import torch
assert torch.cuda.is_available(), "CUDA unavailable: A2+SE1+MMD must run on GPU."
...
GPU_ID="${GPU_ID:-0}"
RESULTS_SUBDIR="${RESULTS_SUBDIR:-20260324_A2PlusSE1_MMD_fold1}"
ROUND_DIR="${PROJECT_ROOT}/results/${RESULTS_SUBDIR}"
RUNTIME_DIR="${PROJECT_ROOT}/dataset/passlevel_tree_select/runtime_rms7_plus_feat4_plus_se1_a2_mmd_fold1_gpu${GPU_ID}"
...
model_id="PHM_c1c4_to_c6_rms7_plus_feat4_plus_se1_A2MMD_dual_seed2026_e${TRAIN_EPOCHS:-1000}_bt96_gpu${GPU_ID}"
